...
from utils.utils import get_data_info, load_data, load_word_embeddings, get_lex_file_list, load_bin_vec
from models.LWAAN_model import LWAAN_model
...
if __name__ == '__main__':
    ...
    # tf.app.flags.DEFINE_string('embedding_fname', './vec/googlew2v.bin', 'embedding file name')
    ...
    if FLAGS.domain == 'twitter' or FLAGS.domain == '14semeval_rest' or FLAGS.domain == '14semeval_laptop':
        ...
        train_aspect_lens, train_context_lens = [len(x["twords"]) for x in train_set], [len(x["words"]) for x in train_set]
    ...
    lex = LexHelper(lex_list, train, test, max_aspect_len=data["max_aspect_len"], max_context_len=data["max_context_len"])

    if FLAGS.lex_embedd_path == '':
        train_context_lex, train_aspect_lex, test_context_lex, test_aspect_lex, data["lex_dim"] = lex.build_lex_embeddings()
    else:
