...
from models.WAAN_model import WAAN_model
...
if __name__ == '__main__':
    ...
    if FLAGS.domain == 'twitter' or FLAGS.domain == '14semeval_rest' or FLAGS.domain == '14semeval_laptop':
        ...
        train_aspects, train_contexts, train_labels, \
        train_aspect_lens, train_context_lens = read_data(FLAGS.train_fname, data["word2id"], data["max_aspect_len"],
                                                          data["max_context_len"],
                                                          FLAGS.train_data, FLAGS.pre_processed)

        test_aspects, test_contexts, test_labels, \
        test_aspect_lens, test_context_lens = read_data(FLAGS.test_fname, data["word2id"], data["max_aspect_len"],
                                                        data["max_context_len"], FLAGS.test_data,
                                                        FLAGS.pre_processed)
    ...
    with tf.Session() as sess:
        model = WAAN_model(FLAGS, sess, data)
        model.build_model()
        model.train(train_data, test_data)

    print("model=AAN, embedding=%s, batch-size=%s, n_epoch=%s, n_hidden=%s, domain=%s" % (
