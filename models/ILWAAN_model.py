...
class ILWAAN_model(object):
    ...
    def build_model(self):
        ...
        with tf.name_scope('LSTM'):
            ...
            context_avg_outputs_iter = tf.TensorArray(tf.float32, 1, dynamic_size=True, infer_shape=False)
            context_avg_outputs_iter = context_avg_outputs_iter.unstack(context_outputs)
            ...
            def _body(i, context_avg_rep, context_avg):
                a = context_avg_outputs_iter.read(i)
            ...
            self.context_avgs = tf.reshape(context_avg_final.stack(), [-1, self.max_context_len])
            self.context_avg_reps = tf.reshape(context_avg_rep_final.stack(), [-1, self.n_hidden])
            ########################################
            ########################################
        ...
        with tf.name_scope('summary'):
            ...
            self.out_dir = 'logs/' + str(timestamp) + '_r' + str(self.learning_rate) + '_b' + str(
                ...
                self.l2_reg) + '_IALSTM_LEX' + '_' + str(self.domain)
    ...
    def eval_step(self, data):
        ...
        for sample, num in self.get_batch_data(aspects, contexts, labels, aspect_lens, context_lens, aspect_lex, context_lex, len(data), False,
            ...
            loss, accuracy, step, summary, predicted, actual = self.sess.run(
                [self.loss, self.accuracy, self.global_step, self.test_summary_op, self.predicted, self.actual],
                feed_dict=sample)
    ...
    def get_batch_data(self, aspects, contexts, labels, aspect_lens, context_lens, aspect_lex, context_lex, batch_size, is_shuffle, keep_prob):
