    ...
    def build_model(self):
        ...
        with tf.name_scope('tracking'):
            ...
            self.out_dir = 'logs/' + str(timestamp) + '_r' + str(self.learning_rate) + '_b' + str(self.batch_size) + '_h' + \
                           str(self.n_hidden) + '_e' + str(self.n_epoch) + str(self.domain)
    ...
    def train(self, sess, train_data, test_data):
        ...
        for i in range(self.n_epoch):
            ...
            print('epoch %s: train-loss=%.6f; train-acc=%.6f; train-loss=%.6f; test-acc=%.6f; best-acc=%.6f; precision=%.6f; recall=%.6f'
                  '; f1-score=%.6f' % (str(i), train_loss, train_acc, test_loss, test_acc, max_acc, precision, recall, f1_score))

        # saver.save(self.sess, self.checkpoint_prefix)
        print('The max accuracy of testing results is %s of step %s \n' % (max_acc, current_step))
